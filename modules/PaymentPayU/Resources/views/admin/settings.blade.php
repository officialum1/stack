<div class="space-y-5">
    <div class="space-y-4 rounded-[1rem] border p-5" style="border-color: var(--theme-border-color); background: var(--theme-surface-soft);">
        <div>
            ...
            <p class="mt-2 text-sm leading-6" style="color: var(--theme-muted-text-color);">{{ __('Control PayU availability and provide the merchant key and salt used for hosted checkout.') }}</p>
        </div>

        <div class="grid gap-5 md:grid-cols-2">
            <x-ui.radio-group name="state.payu_status" wire:model.defer="state.payu_status" :label="__('Status')" :options="$toggleOptions" :value="data_get($state, 'payu_status')" />
            <x-ui.radio-group name="state.payu_environment" wire:model.defer="state.payu_environment" :label="__('Environment')" :options="$environmentOptions" :value="data_get($state, 'payu_environment')" />
        </div>

        <div class="grid gap-5 md:grid-cols-2">
            <x-ui.input wire:model.defer="state.payu_merchant_key" :label="__('Merchant Key')" type="text" :error="$errors->first('state.payu_merchant_key')" />
            <x-ui.password-input wire:model.defer="state.payu_salt" :label="__('Salt')" :error="$errors->first('state.payu_salt')" :placeholder="__('Paste PayU salt')" />
        </div>
    ...
    <div class="space-y-4 rounded-[1rem] border p-5" style="border-color: var(--theme-border-color); background: var(--theme-surface-soft);">
        <div>
            ...
            <p class="mt-2 text-sm leading-6" style="color: var(--theme-muted-text-color);">{{ __('Register these URLs in your PayU dashboard.') }}</p>
        </div>
    ...
    <div class="space-y-5 rounded-[1rem] border p-5" style="border-color: var(--theme-border-color); background: var(--theme-surface-soft);">
        <div>
            ...
            <p class="mt-2 text-sm leading-6" style="color: var(--theme-muted-text-color);">{{ __('PayU hosted checkout must return to the app on a public HTTPS URL and the response hash must verify successfully.') }}</p>
        </div>
