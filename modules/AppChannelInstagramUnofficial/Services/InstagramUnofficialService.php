...
class InstagramUnofficialService
{
    ...
    public function authenticate(string $username, string $password): array
    {
        try {
            ...
            $authorization = $this->normalizeHeaderValue($response->header('ig-set-authorization'));
    ...
    public function verifyTwoFactorCode(string $two_factor_identifier, string $code, string $verification_method = '1'): array
    {
        try {
            ...
            $auth = $this->normalizeHeaderValue($response->header('ig-set-authorization'));
            $body = $response->json();
    ...
    public function submitSecurityCode(string $apiPath, string $securityCode): array
    {
        ...
        try {
            ...
            if (!empty($json['status']) && $json['status'] === 'ok' && !empty($json['logged_in_user'])) {
                ...
                $this->authData['authorization'] = $this->normalizeHeaderValue($response->header('ig-set-authorization'));
                return [
                    'status' => true,
                    'data'   => [
    ...
    {
        if (empty($filePathOrUrl) || !is_string($filePathOrUrl)) {
            return [$filePathOrUrl, $filePathOrUrl, false];
        }

        if (filter_var($filePathOrUrl, FILTER_VALIDATE_URL)) {
            $tempPath = $this->downloadTempFile($filePathOrUrl, $defaultExt);

            return [$tempPath, $tempPath, true];
        }

        if (file_exists($filePathOrUrl) && is_readable($filePathOrUrl)) {
            return [ $filePathOrUrl, $filePathOrUrl, false ];
        }

        $storagePath = \Media::path($filePathOrUrl);
        if ($storagePath && file_exists($storagePath) && is_readable($storagePath)) {
            return [ $storagePath, $filePathOrUrl, false ];
        }

        $mediaUrl = \Media::url($filePathOrUrl);
        if ($mediaUrl && filter_var($mediaUrl, FILTER_VALIDATE_URL)) {
            $tempPath = $this->downloadTempFile($mediaUrl, $defaultExt);
    ...
    protected function downloadTempFile(string $url, string $defaultExt = 'jpg'): string
    {
        $response = Http::timeout(60)->get($url);

        if (! $response->successful()) {
            throw new \RuntimeException('Could not fetch the remote Instagram media file.');
        }

        $extension = strtolower(pathinfo(parse_url($url, PHP_URL_PATH) ?: '', PATHINFO_EXTENSION));
        if ($extension === '') {
            $extension = strtolower($defaultExt);
        }

        $directory = 'temp_instagram/'.date('Y/m');
    ...
    protected function normalizeHeaderValue($value): string
    {
        if (is_array($value)) {
