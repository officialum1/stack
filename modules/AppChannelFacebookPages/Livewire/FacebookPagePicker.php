...
namespace Modules\AppChannelFacebookPages\Livewire;
...
class FacebookPagePicker extends Component
{
    ...
    {
        $this->context = (array) session('facebook_page_picker', []);
        $this->pages = array_values((array) ($this->context['pages'] ?? []));

        abort_unless($this->planAccess()->canCreate(auth()->user()) && $this->planAccess()->canUseCapability(auth()->user(), 'facebook_page'), 403);
    }
    ...
    public function addSelected(array $selectedIds = [])
    {
        ...
        $remainingSlots = $this->planAccess()->remainingSlots(auth()->user(), 'facebook_page');
    ...
    protected function persistSelected(array $selectedIds = [])
    {
        ...
        foreach ($pages as $page) {
            if (! in_array((string) $page['id'], $existingIds, true)
                && $this->planAccess()->hasReachedLimit(auth()->user(), 'facebook_page')) {
                break;
            }
            ...
            SocialAccount::query()->updateOrCreate(
                ...
                [
                    ...
                    'username' => Str::slug((string) $page['name']),
        ...
        session()->forget('facebook_page_picker');

        return redirect()
            ->route('portal.channels', ['provider' => 'facebook'])
            ->with('channels.flash', [
                'tone' => 'success',
                'message' => trans_choice(':count Facebook page connected.|:count Facebook pages connected.', $pages->count(), ['count' => $pages->count()]),
            ]);
    }

    public function reconnect()
    {
        return redirect()->route('portal.channels.facebook.pages.connect');
    }
    ...
    public function render(): View
    {
        $items = $this->filteredPages()
            ->map(fn (array $page): array => [
                ...
                'subtitle' => (string) ($page['category'] ?? 'Page'),
                'avatar_url' => (string) ($page['avatar_url'] ?? ''),
            ])->values()->all();

        return view('appchannelfacebookpages::livewire.facebook-page-picker', [
            'items' => $items,
            'hasSourceItems' => $this->pages !== [],
            'sourceItemsCount' => count($this->pages),
            'selectedCount' => count($this->selectedIds),
            'pickerEyebrow' => __('Channel connection'),
            'pickerTitle' => __('Select channels'),
            'pickerDescription' => __('Choose only the Facebook pages you want to connect to this workspace.'),
            'reconnectLabel' => __('Reconnect'),
            'emptyTitle' => __('No Facebook pages found'),
            'emptyDescription' => __('Reconnect your Facebook account to refresh the available pages, then pick the ones you want to add.'),
            'emptyBody' => __('No selectable pages came back from Facebook for this account.'),
            'collectionTitle' => __('Facebook pages'),
            'collectionDescription' => __('Review the pages returned by Facebook and add only the ones you want in this workspace.'),
            'searchLabel' => __('Search pages'),
            'searchHelp' => __('Filter by page name or category before adding.'),
        ...
        return collect($this->pages)
            ->filter(function (array $page) use ($applySearch): bool {
                ...
                return Str::contains(Str::lower((string) ($page['name'] ?? '')), $needle)
                    || Str::contains(Str::lower((string) ($page['category'] ?? '')), $needle);
            });
    }
