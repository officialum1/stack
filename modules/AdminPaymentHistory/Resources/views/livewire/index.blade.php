...
<div class="space-y-6">
    ...
    <x-ui.page-hero
        ...
        :description="__('Review transactions, payment gateways, invoice references, and plan assignments.')"
        :count="$summary['total']"
    ...
    <x-ui.filter-panel fields-class="xl:grid-cols-[minmax(0,1.35fr)_220px_220px_auto]">
        <x-slot:fields>
            <x-ui.input wire:model.live.debounce.300ms="search" :label="__('Search')" :placeholder="__('Invoice, transaction, user, gateway...')" />
    ...
    <x-ui.datatable-shell
        ...
        :info="__('Read-only administrative history for completed, pending, and refunded plan payments.')"
        header-class="py-4"
    ...
    >
        <x-ui.table class="rounded-none border-0 shadow-none">
            ...
            <x-ui.table-body>
                @forelse ($payments as $payment)
                    <x-ui.table-row wire:key="payment-history-row-{{ $payment->id }}">
                        <x-ui.table-cell><div class="space-y-1"><p class="font-semibold uppercase" style="color: var(--theme-header-text-color);">{{ $payment->id_secure ?: 'N/A' }}</p><p class="text-xs" style="color: var(--theme-muted-text-color);">#{{ $payment->id }}</p></div></x-ui.table-cell>
                        <x-ui.table-cell><div class="space-y-1"><p class="font-medium" style="color: var(--theme-header-text-color);">{{ $payment->user?->name ?: __('Unknown user') }}</p><p class="text-xs" style="color: var(--theme-muted-text-color);">{{ $payment->user?->email ?: __('No email') }}</p></div></x-ui.table-cell>
                        ...
                        <x-ui.table-cell>
                            <x-ui.dialog wire:key="payment-history-delete-dialog-{{ $payment->id }}" :title="__('Delete this payment history record?')" :description="__('This permanently removes the selected payment history row.')" width="sm" dismissible>
                                <x-slot:trigger><x-ui.button type="button" variant="danger" size="sm">{{ __('Delete') }}</x-ui.button></x-slot:trigger>
                                <x-slot:footer>
                                    <div class="flex justify-end gap-3">
                                        <x-ui.button type="button" variant="outline" x-on:click="open = false">{{ __('Cancel') }}</x-ui.button>
                                        <x-ui.button type="button" variant="danger" wire:click="delete({{ $payment->id }})" x-on:click="open = false">{{ __('Delete') }}</x-ui.button>
                                    </div>
                                </x-slot:footer>
                            </x-ui.dialog>
                        </x-ui.table-cell>
                ...
                @empty
                    <x-ui.table-row>
                        <x-ui.table-cell colspan="9" class="py-10">
                            <x-ui.empty icon="fa-light fa-receipt" :title="__('No payment history records found.')" :description="__('Transactions will appear here once successful, pending, or refunded payments are recorded.')" />
                        </x-ui.table-cell>
                    </x-ui.table-row>
                @endforelse
            </x-ui.table-body>
        ...
        @if ($payments->hasPages())
            <div class="border-t px-6 py-4" style="border-color: var(--theme-border-color);">{{ $payments->links() }}</div>
        @endif
    </x-ui.datatable-shell>
</div>
