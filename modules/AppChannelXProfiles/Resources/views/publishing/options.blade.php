@php($currentType = (string) data_get($composer, 'network_options.'.$providerKey.'.x_post_type', 'auto'))

<div class="space-y-4 px-4 py-4" x-data="{ postType: @js($currentType) }">
    <div class="space-y-2.5">
        <x-ui.label>{{ __('X Post Type') }}</x-ui.label>
        <div class="flex flex-wrap gap-2">
            @foreach ([
                ...
                <button
                    ...
                    x-on:click="
                        postType = '{{ $option['key'] }}';
                        $wire.set('composer.network_options.{{ $providerKey }}.x_post_type', '{{ $option['key'] }}', false);
                    "
